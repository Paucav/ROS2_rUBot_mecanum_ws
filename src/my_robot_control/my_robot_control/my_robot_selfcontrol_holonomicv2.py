...
class RobotSelfControl(Node):
    ...
    def laser_callback(self, scan):
        ...
        # React to obstacle: comportament de "rebot" diagonal sense rotacions

        if closest_distance < self._distanceLimit:
            self._msg.angular.z = 0.0

            if zone == "FRONT":
                # obstacle davant -> invertir component X, mantenir Y
                self._msg.linear.x = -self._msg.linear.x
            
            elif zone == "BACK_LEFT" or zone == "BACK_RIGHT":
                # darrere -> tractem com a BACK: invertir X
                self._msg.linear.x = -self._msg.linear.x
            
            elif zone == "LEFT":
                # obstacle a l'esquerra -> conservar X, invertir Y
                self._msg.linear.y = -self._msg.linear.y
            
            elif zone == "RIGHT":
                # obstacle a la dreta -> conservar X, invertir Y
                self._msg.linear.y = -self._msg.linear.y
            
            else:
                # fora de FOV: mantenir el que hi ha
                pass
        
            # Assegurem la magnitud de les components (preservant signe)
            ...
            # No obstacle dins del límit -> mantenim moviment diagonal
            # Preserve sign but enforce configured speed magnitude
            self._msg.angular.z = 0.0
            # Si alguna component està a zero (teòricament no hauria), assignem la direcció per defecte
            if self._msg.linear.x == 0.0:
                self._msg.linear.x = self._forwardSpeed * self._speedFactor
            if self._msg.linear.y == 0.0:
                self._msg.linear.y = -self._forwardSpeed * self._speedFactor
            # Ajustem magnitud però preservem signes actuals
            self._msg.linear.x = math.copysign(self._forwardSpeed * self._speedFactor, self._msg.linear.x)
            self._msg.linear.y = math.copysign(self._forwardSpeed * self._speedFactor, self._msg.linear.y)
