...
class WallFollower(Node):
    ...
    def laser_callback(self, scan):
        ...
        #----------------------------------------------------------
        # RULE 4: BACK-RIGHT / BACK → diagonal holonómica para rodear
        #         (sin giros raros, solo movimiento en x,y)
        #----------------------------------------------------------
        elif math.isfinite(min_back_right) or math.isfinite(min_back):
            # Mantener un movimiento claro de rodeo:
            # hacia atrás + hacia la izquierda (diagonal),
            # para que el robot pase por detrás del objeto azul
            # y luego la RULE 3 coja el control cuando la pared
            # vuelva a aparecer en RIGHT.
            twist.linear.x = -self.v_lin * 0.3   # un poco hacia atrás
            twist.linear.y = self.v_lin          # fuerte hacia la izquierda
            twist.angular.z = 0.0
            ref_dist = min(min_back_right, min_back)
            action = (
                f"BACK/BACK-RIGHT {ref_dist:.2f} m → DIAGONAL BACK+LEFT (rodeando)"
            )
